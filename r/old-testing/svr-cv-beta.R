...
for (fold in folds)
{
  ...
  for (my.cost in costs)
  {
    ...
    {
      model <- svm(train.beta~distance_to_grain_boundary+unit_vector_to_grain_boundary_x+unit_vector_to_grain_boundary_y+unit_vector_to_grain_boundary_z+phi1+phi2+misorientation+scaled_angle, data=train.input, kernel ="radial", 
  ...
  train.mse.beta <- vector(mode = "list", length = num.models)
  tune.mse.beta <- vector(mode = "list", length = num.models)
  ...
  {
    train.pred.beta = predict(models[[i]], train.input)
    train.mse = sum((train.pred.beta - train.beta)^2) / length(train.beta)
    tune.pred.beta = predict(models[[i]], tune.input)
    tune.mse = sum((tune.pred.beta - tune.beta)^2) / length(tune.beta)
    
    train.mse.beta[i] = train.mse
    tune.mse.beta[i] = tune.mse
  ...
  cv.to.plot[[fold]] = matrix(tune.mse.beta, nrow=length(gammas), ncol=length(costs))
  
  test.pred.beta = predict(models[[minimum.index]], test.input)
  test.mse.beta = sum((test.pred.beta - test.beta)^2) / length(test.beta)
  cv.test.mses[fold] = test.mse.beta
}
