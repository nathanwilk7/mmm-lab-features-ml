...
for (fold in folds)
{
  ...
  train.mse.cz <- vector(mode = "list", length = num.models)
  tune.mse.cz <- vector(mode = "list", length = num.models)
  ...
  {
    train.pred.cz = predict(models[[i]], train.input)
    train.mse = sum((train.pred.cz - train.cz)^2) / length(train.cz)
    tune.pred.cz = predict(models[[i]], tune.input)
    tune.mse = sum((tune.pred.cz - tune.cz)^2) / length(tune.cz)
    
    train.mse.cz[i] = train.mse
    tune.mse.cz[i] = tune.mse
  ...
  cv.to.plot[[fold]] = matrix(tune.mse.cz, nrow=length(gammas), ncol=length(costs))
  
  test.pred.cz = predict(models[[minimum.index]], test.input)
  test.mse.cz = sum((test.pred.cz - test.cz)^2) / length(test.cz)
  cv.test.mses[fold] = test.mse.cz
}
