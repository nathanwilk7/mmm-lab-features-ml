...
train.error.beta <- vector(mode = "list", length = 7*7)
test.error.beta <- vector(mode = "list", length = 7*7)
...
for (i in 1:(7*7))
{
  ...
  train.error.beta[i] = train.error
  test.error.beta[i] = test.error
...
test.error.beta
to.plot = matrix(test.error.beta, nrow=7, ncol=7)
levelplot(to.plot, at=c(.7,.8,.9,1,1.1,1.2), ylab='Costs', xlab='Gammas')
test.error.beta[minimum.index]
costs[minimum.index]
gammas[minimum.index]

plot(1:49,test.error.beta)
abline(h=test.error.beta[minimum.index], col='red', lwd=3)
print(minimum.index)
print(test.error.beta[minimum.index])
